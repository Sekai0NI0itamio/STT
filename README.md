...
The default v1 backend runs `faster-whisper` on GitHub-hosted Ubuntu runners in direct full-file mode for committed `.mp3` inputs, so the model can use the full file without first exporting silence-cut subclips. The chunked silence-aware path still exists as an override when you want it. The workflow reuses cached Python environments and model files between runs when possible instead of reinstalling everything every time, and it automatically avoids oversubscribing CPU-heavy `medium` and larger models with too many simultaneous transcribes.
